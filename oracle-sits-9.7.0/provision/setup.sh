...
PATH=$JAVA_HOME/bin:$PATH:.:$ORACLE_HOME/bin ; export PATH

mkdir ${DIR_TEMP}
LD_LIBRARY_PATH=$JAVA_HOME/lib/amd64/server:${DIR_TEMP}/uniface/lib:$LD_LIBRARY_PATH:$ORACLE_HOME/lib ; export LD_LIBRARY_PATH


# Extract Uniface
unzip -qn ${ZIP_UNIFACE} 'uniface970lid.tar.gz' -d ${DIR_TEMP}/uniface
tar --strip-components=1 -xzf ${DIR_TEMP}/uniface/uniface970lid.tar.gz -C ${DIR_TEMP}/uniface

# Extract Uniface licence from SITS
unzip -qn ${ZIP_SITS} 'sits970.zip' -d ${DIR_TEMP}
unzip -qn ${DIR_TEMP}/sits970.zip 'release/9.7.0/sits/cwlm/license9.xml' -d ${DIR_TEMP}
mv ${DIR_TEMP}/release/9.7.0/sits/cwlm/license9.xml ${DIR_TEMP}/license9.xml
...
su - oracle -c "dbca -silent -responseFile $DIR_PROVISION/dbca-sits.rsp"
...
make -f ins_rdbms.mk lbac_off   # Turn off 'Oracle Label Security'
...
@${DIR_METADATA}/sits_metadata.SYNONYM.sql
...
@${DIR_METADATA}/sits_metadata.TABLE.INDEX.FUNCTIONAL_AND_BITMAP.INDEX.sql
...
# Import SITS 9.7.0 release data
cd ${DIR_TEMP}/reldata_970
